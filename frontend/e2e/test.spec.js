...
// Agent creation test
test("Agent Creation: submits 5 and shows success message", async ({
  page,
}) => {
  await page.goto(APP_URL);
  await page.fill('input[placeholder="Enter a number (1–100)"]', "5");
  await page.getByRole("button", { name: "Submit" }).click();

  await expect(page.getByText("5 agents created successfully! ✅"), {
    timeout: 10000,
  }).toBeVisible();
});

// Dashboard navigation test
test("Dashboard button takes you to /future", async ({ page }) => {
  await page.goto(APP_URL);
  await page.fill('input[placeholder="Enter a number (1–100)"]', "5");
  await page.getByRole("button", { name: "Submit" }).click();
  await expect(page.getByText("5 agents created successfully! ✅"), {
...
// CSV upload test
test("CSV Upload Shows CSV-loaded and unlocks Present answers", async ({
  page,
}) => {
  await page.goto(APP_URL);
  await page.fill('input[placeholder="Enter a number (1–100)"]', "5");
...
test("Future Scenario upload successful and unlocks Future Scenario answers", async ({
  page,
}) => {
  await page.goto(APP_URL);
  await page.fill('input[placeholder="Enter a number (1–100)"]', "5");
  ...
  const futureBtn = page.getByRole("button", {
    name: "Future Scenario Answers",
  });
...
// CSV download test
test("CSV download works and contains correct files with correct data", async ({
  page,
}) => {
  await page.goto(APP_URL);
  await page.fill('input[placeholder="Enter a number (1–100)"]', "3");
  await page.getByRole("button", { name: "Submit" }).click();
  await expect(page.getByText("3 agents created successfully! ✅"), {
    timeout: 10000,
  }).toBeVisible();
  await page.getByRole("button", { name: "Dashboard 📈" }).click();
  await expect(page).toHaveURL(`${APP_URL}/future`);
