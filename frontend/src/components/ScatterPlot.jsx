...
const ScatterPlot = ({ data, xAxis }) => {
  ...
  useEffect(() => {
    ...
    const svg = d3
      .select(svgRef.current) // Select the SVG element using a reference
    ...
    const xScale = isCategorical
      ? d3
          .scalePoint() //uses scalepoint to display options as male or female
          .domain(["Male", "Female"])
          .range([margin.left + 50, width - margin.right - 50])
          .padding(0, 5)
      : d3
          .scaleLinear() //uses scaleLinear to show values on x-axis
          .domain([0, d3.max(data, (d) => d[xAxis])])
          .range([margin.left, width - margin.right - 25]);

    const yScale = d3
      .scaleLinear() //uses scaleLinear to show 1-10 on the y-axis
      .domain([1, 10])
    ...
    svg
      .append("g") //adds the x-axis to svg
      .attr("transform", `translate(0, ${height - margin.bottom})`)
      .call(xAxisElement);

    svg
      .append("g") // adds the y-axis to svg
    ...
    svg
      ...
      .on("mouseover", (event, d) => {
        tooltip
          .style("display", "block")
          .html(
            `Agent: ${d.name} <br> ${xAxis}: ${d[xAxis]} <br> Response: ${d.response}`,
          )
